...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("SAHAL ARBANI Update Available!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[SAHAL ARBANI] Status: Latest version")
        end
    else
        warn("[SAHAL ARBANI] Status unknown:", response)
    end
end
...
if rodData.Data and rodData.Data.Type == "Fishing Rods" then
    ...
    -- 1. LUCK: Dikali 8
    -- Hasil: 1.325% -> 10.600% (Auto Panen Mythical)
    if rodData.RollData and rodData.RollData.BaseLuck then
        rodData.RollData.BaseLuck *= 8 
    end

    -- 2. POWER & RESILIENCE: Dikali 3 (Dinaikkan dari versi sebelumnya)
    -- Karena Luck 10.000% pasti dapat ikan "Boss/Raksasa",
    -- kita butuh power x3 biar senar gak meledak/putus.
    ...
    if rodData.MaxWeight then
        rodData.MaxWeight *= 5 -- Kapasitas berat x5 (Jaga-jaga dapat ikan paus)
    end

    -- 3. SPEED: Dikali 0.7 (Tetap 30% lebih cepat biar enak spam)
    if typeof(rodData.Windup) == "NumberRange" then
        local newMin = rodData.Windup.Min * 0.7
        local newMax = rodData.Windup.Max * 0.7
        rodData.Windup = NumberRange.new(newMin, newMax)
    end

    NotifySuccess("GOD LUCK ACTIVE", "Luck: ~" .. (rodData.RollData.BaseLuck) .. " (x8 Boost)", 3)
else
