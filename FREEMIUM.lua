...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("WILSONxARB Update Available!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[WILSONxARB] Status: Latest version")
        end
    else
        warn("[WILSONxARB] Status unknown:", response)
    end
end
...
local function modifyRodData(rodNameInput)
    ...
    if rodData.Data and rodData.Data.Type == "Fishing Rods" then
        ...
        NotifySuccess("Rod Modified", "Rod '" .. rodData.Data.Name .. "' successfully boosted x8.", 3)
    else
